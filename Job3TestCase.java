    ...
    public static void main(String[] args) throws IOException {
        ...
        while (i < 3 && count < maxTries) {
            try {
                ...
                PrintStream output = new PrintStream(new FileOutputStream("outputJ3test.txt"));
                ...
                output.print("Job: ");
                jobType = "3";// jobType is 3 for test case of job 3
                System.out.println(jobType);
                output.println(jobType);
                pr.println(jobType);
                pr.flush();
                //get ip address from user if not job 3, otherwise ip is localhost
                if (!jobType.equalsIgnoreCase("3")) {
                    System.out.print("Enter an IP Address/host name: ");
                    output.print("Enter an IP Address/host name: ");
                    ip = "www.google.com";// hostname for test case
                    output.println(ip);
                }
                else {
                    System.out.print("JobSeeker IP Address: ");
                    output.print("JobSeeker IP Address: ");
                    ip = (InetAddress.getLocalHost().getHostAddress()).trim();//local IP address, same for test case
